...
def process_frame(im, width, height, n_clusters=16, prescale_size=None, return_scaled = True):
    h, w = im.shape[:2]
    ...
    im0 = im_quantize(im, n_clusters)

    # Quantize (cluster colors) and resample (subsample+supersample)
    im1,im2 = im_resample(im0, width, height, w, h)
...
def im_resample(im, subsample_width, subsample_height, resample_width, resample_height):
    ...
    im_subsample = cv2.resize(im, (subsample_width, subsample_height), interpolation=cv2.INTER_NEAREST)
    im_rescale = cv2.resize(im_subsample, (resample_width, resample_height), interpolation=cv2.INTER_NEAREST)

    return (im_subsample, im_rescale)
